...
export function InternetIdentityProvider({
  ...
  // Use a ref for the auth client to avoid re-running the init effect when the
  // client is set. State is only used for values that drive rendering.
  ...
  // Initialize exactly once. Using a ref guard so the effect never re-runs due
  // to authClient state changes (which was causing double initialization).
  useEffect(() => {
